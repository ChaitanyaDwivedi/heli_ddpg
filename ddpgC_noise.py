...
def train(sess, env, args, actor, critic, actor_noise):
    ...
    for i in range(int(args['max_episodes'])):
        print("reseting, episode: "+ str(i) + " started")
        s = env.reset()
        ...
        for j in range(int(args['max_episode_len'])):
            ...
            s2,r, terminal = env.step(a[0][0], a[0][1], s)
            #print(a,s,s2,r)
            if(j%100==1):
                print(a[0][0], a[0][1])
                print([x*57 for x in s],[x*57 for x in s2],r*57)
...
def main(args):
    ...
    with tf.Session() as sess:
        ...
        state_dim = 3
        action_dim = 2
        action_bound = env.action_space_high
        # Ensure action bound is symmetric
        assert (env.action_space_high == -env.action_space_low)
...
if __name__ == '__main__':
    ...
    pp.pprint(args)
    #args = {'actor_lr': 0.0001, 'tau': 0.001, 'minibatch_size': 64, 'critic_lr': 0.001, 'gamma': 0.99, 'summary_dir': './results/tf_ddpg'   }
    main(args)
